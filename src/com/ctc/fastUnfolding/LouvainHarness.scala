...
class LouvainHarness(minProgress: Int, progressCounter: Int) {
  ...
  def run[VD: ClassTag](sc: SparkContext, graph: Graph[VD, Long]) = {
    ...
    var louvainGraph = Core.createLouvainGraph(graph)
    ...
    do {
      ...
      val (currentQ, currentGraph, passes) = Core.louvain(sc, louvainGraph, minProgress, progressCounter)
      ...
      if (passes > 2 && currentQ > q + 0.001) {
        q = currentQ
        louvainGraph = Core.compressGraph(louvainGraph)
      }
      else {
        halt = true
      }
