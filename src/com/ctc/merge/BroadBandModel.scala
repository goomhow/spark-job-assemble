...
class BroadBandModel(@transient spark:SparkSession)extends Serializable {
  ...
  val DAN_BD_LABEL_PATH = "dan/bd_label/%s"
  def bdDanFlag(month:String): DataFrame ={
    ...
    bd.join(cdma,Seq("ACCT_ID"),"left").where("PRD_INST_ID_A IS NULL").select("PRD_INST_ID_B").toDF(ID_COLUMN)
  }
  def getLabel(month:String,update:Boolean=false):DataFrame = {
    val path = DAN_BD_LABEL_PATH.format(month)
    if(update || !exists(path)){
      def f0 = udf(()=>0)
      def f1 = udf(()=>1)
      val dan3 = bdDanFlag(month)
      val m4 = next_month(month,1)
      val df =if(exists(BD_DATA_PATH.format(m4))){
        val dan4 = bdDanFlag(m4).withColumn("LABEL",f0())
  ...
  def checkModel(idPath:String,predict_month:String): Unit ={
    ...
    val td_tabel = "td_work.dan_bd_zr"
    val t_sql = s"""sel a.PRD_INST_ID from $td_tabel a
                  inner join (select prd_inst_id,ofr_id from PV_MART_Z.bas_prd_inst_month
                  where billing_cycle_id=$job_month and Std_Prd_Inst_Stat_Id/100<>12 and substr(trim(std_prd_id),1,4)=3020
